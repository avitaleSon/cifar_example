...
if __name__ == '__main__':
    """
    This script loads the CIFAR-10 dataset and runs classification using a ResNet-18.
    ...
    # Create model
    layers = [3,4,6,3] # ResNet-18
    resnet_model = ResNet(ResidualBlock,
                          layers=layers)
    ...
    mlflow.set_experiment("cifar10_resnet_experiment")
    ...
    with mlflow.start_run(run_name='ResNet Training') as run:
        ...
        with open("../models/resnet_summary.txt", "w") as f:
            f.write(str(summary(resnet_model)))
        mlflow.log_artifact("../models/resnet_summary.txt")
        ...
        for i_epoch in range(num_epochs):
            ...
            for i, data in enumerate(trainloader,0):
                ...
                if i%1==0:
                   # Log metrics to MLFlow
                    mlflow.log_metric(key="train_loss", value=loss_value.item(), step=i)
            ...
            for i, data in enumerate(validloader, 0):
                ...
                if loss_value.item() < best_valid_loss:
                    ...
                    is_best = True
                    file_path = '../models/resnet_model.pth.tar'
                    save_checkpoint({
                        'epoch': i_epoch + 1,
                    ...
                    if Path("../models/resnet_model").exists():
                        shutil.rmtree("../models/resnet_model")
                    mlflow.pytorch.save_model(resnet_model, "../models/resnet_model")
